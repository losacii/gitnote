...
def imageOver(baseImage, logoImage, x=0, y=0):
    ...
    X = x + 30; Y = y + 30;
    if baseImageWidth - x < 30:  X = baseImageWidth
    if baseImageHeight - y < 30: Y = baseImageHeight
...
TOP = 18; LEFT = 0; WIDTH = 1366; HEIGHT = 732
...
def grabScreen():
    ...
    while True:
        ...
        x, y = pag.position()
        _x, _y = x - LEFT, y - TOP
        if _x <= 0 or _x > WIDTH-30: _x = 0
        if _y <= 0 or _y > HEIGHT-30: _y = 0
        if _x != 0 and _y != 0:
        ...
        elif key == ord('f') or (x > screen_width - 25): # _ _ _ _ _ _ _ _ _ _ _ _  Fast mode: F
            print("==> fast mode!")
            fastMode = True
        elif key == ord('n') or (x < 25): # _ _ _ _ _ _ _ _ _ _ _ _  Normal mode: N
            fastMode = False
            print("==> normal mode.")
